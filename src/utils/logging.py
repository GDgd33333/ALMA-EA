...
class Logger:
    ...
    def print_recent_stats(self):
        ...
        for (k, v) in sorted(self.stats.items()):
            ...
            # Convert tensors to numpy if needed (handle CUDA tensors)
            values = [x[1].cpu().numpy() if hasattr(x[1], 'cpu') else x[1] for x in self.stats[k][-window:]]
    ...
    def print_stats_summary(self):
        ...
        for (k, v) in sorted(self.stats.items()):
            ...
            i += 1
            # Convert tensors to numpy if needed (handle CUDA tensors)
            values = [x[1].cpu().numpy() if hasattr(x[1], 'cpu') else x[1] for x in self.stats[k]]
