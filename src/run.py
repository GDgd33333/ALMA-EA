...
def run_sequential(args, logger):
    ...
    if hasattr(args, 'use_ea') and args.use_ea and args.hier_agent["task_allocation"] == "aql":
        ...
        ea_manager = EAManager(
            population_size=ea_config.get('population_size', 5),  # 种群大小，默认5
            elite_size=ea_config.get('elite_size', 2),  # 精英个体数量，默认2
            mutation_rate=ea_config.get('mutation_rate', 0.1),  # 变异率，默认0.1
            mutation_strength=ea_config.get('mutation_strength', 0.1),  # 变异强度，默认0.1
            crossover_rate=ea_config.get('crossover_rate', 0.8),  # 交叉率，默认0.8
            selection_type=ea_config.get('selection_type', 'tournament'),  # 选择类型，默认锦标赛
            tournament_size=ea_config.get('tournament_size', 3),  # 锦标赛大小，默认3
            evaluation_episodes=ea_config.get('evaluation_episodes', 3),  # 评估回合数，默认3
        ...
        logger.console_logger.info("EA Manager initialized for allocation policy evolution")
    ...
    while runner.t_env <= args.t_max:
        ...
        if ea_manager is not None and (runner.t_env - last_ea_eval_T) >= ea_manager.sync_interval:
            logger.console_logger.info("Starting EA evaluation at t_env: {}".format(runner.t_env))
            
            # 评估种群
            ea_manager.evaluate_population(runner, runner.t_env)
            
            # 评估主网络性能用于双向同步
            main_network_fitness = None
            if ea_manager.enable_bidirectional_sync:
                ...
                logger.console_logger.info(f"Main network fitness: {main_network_fitness:.4f}")
            else:
                logger.console_logger.info("Bidirectional sync disabled")
            
            # 打印进化算法统计信息
            ea_manager.print_stats()
            
            # 进化到下一代，支持双向同步
            logger.console_logger.info(f"Evolving with main_network_fitness={main_network_fitness}, alloc_policy={'available' if mac.alloc_policy is not None else 'None'}")
            ea_manager.evolve_generation(main_network_fitness=main_network_fitness, main_alloc_policy=mac.alloc_policy)
