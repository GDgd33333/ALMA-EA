    ...
    def update(self, data, bs=slice(None), ts=slice(None), mark_filled=True):
        ...
        for k, v in data.items():
            ...
            if self.efficient_store and k in self.data.transition_data:
                _bs, _ts = _slices
                # Allow partial trajectories if they fit within max_seq_length
                # Check if it's a full trajectory or starts from 0 and fits within max_seq_length
                # Note: step=None is equivalent to step=1 in Python slices
                step_val = _ts.step if _ts.step is not None else 1
                assert ((_ts.start, _ts.stop, step_val) == (0, self.max_seq_length, 1) or
                         _ts == slice(None) or
                         (_ts.start == 0 and _ts.stop is not None and _ts.stop <= self.max_seq_length and (step_val == 1 or _ts.step is None))), \
                        f"Efficient store only supports replacing entire trajectories. Got slice: start={_ts.start}, stop={_ts.stop}, step={_ts.step}, expected: start=0, stop={self.max_seq_length}, step=1 or None"
        ...
        # Return data from all keys but cut time to slices
            ...
            if self.efficient_store:
                ...
                assert ((_ts.start, _ts.stop, step_val) == (0, self.max_seq_length, 1) or
                         _ts == slice(None) or
                         (_ts.start == 0 and _ts.stop is not None and _ts.stop <= self.max_seq_length and (step_val == 1 or _ts.step is None))), \
                        f"Efficient store only supports getting entire trajectories. Got slice: start={_ts.start}, stop={_ts.stop}, step={_ts.step}, expected: start=0, stop={self.max_seq_length}, step=1 or None"
                tr_item = _bs
            else:
                tr_item = item
